...
func main() {
	flag.StringVar(&inputURL, "input-url", "", "カメラのRTSP URLまたはRTP SDPファイルパス")
	...
	flag.StringVar(&inputType, "input-type", "rtsp", "入力タイプ (rtsp または rtp)")
	...
	if inputURL == "" {
		log.Fatal("入力URL（RTSPまたはRTP SDPファイル）を指定する必要があります")
	}
	log.Printf("入力URL: %s, 入力タイプ: %s, コーデック: %s, 出力モード: %s を使用します", inputURL, inputType, codec, outputMode) // 変更
	if codec == "h265" {
